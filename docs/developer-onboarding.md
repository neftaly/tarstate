...
- Incremental materialization currently accepts only one base relation with an
  ...
  same subset. It also accepts raw simple inner equality joins of two base
  relations; left joins, non-equality joins, self joins, transformed join inputs,
  and post-join row transforms remain diagnostic-backed.
