...
| Consumer DX | Named `openDatabaseQuery` as the normal multi-source query path and `DatabaseView` as a host seam; added exact import paths, direct mapped-row selection, prepared capabilities, and one-place scope configuration. | A complete third pass found no new public concept, cast, or path to remove. |
