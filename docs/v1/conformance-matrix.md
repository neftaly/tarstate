...
| 6. Source transactions | Complete | `packages/core/tests/production-transaction.test.ts`, `packages/core/tests/transaction-executor.test.ts`, `packages/core/tests/source-protocol.test.ts`, `packages/core/tests/support/restartable-source.ts`, `packages/automerge/tests/mapping-storage-binding.test.ts`, `packages/automerge/tests/core-adapter.test.ts`, and `packages/automerge/tests/production-automerge.test.ts` cover source-specific and generic prepared transaction execution, explicit exclusive/cooperative edit coverage, ordered staging over repeated footprints, staged query bases, explicit capability authorization, simulation, serialization/reentrancy, no-ops, final-state queries and constraints, conflicts, cancellation, epochs/IDs, retained rejection, edit mechanisms, and a serialized reservation/mutation/result recovered after both persistence-shell and source recreation. |
...
| 8. Discovery/bootstrap | Complete | `packages/core/tests/artifact-resolver.test.ts`, `packages/core/tests/attachment-preparation.test.ts`, `packages/core/tests/database-observer.test.ts`, `packages/core/tests/system-relations.test.ts`, `packages/automerge/tests/artifact-resource-driver.test.ts`, and `packages/automerge/tests/metadata.test.ts` cover exact artifact candidates, carrier versus semantic integrity, temporary Repo lease release, alias chains, cycles, missing/denied/stale states, bounded authority caches, bootstrap absence/collision/conflict, the explicit read-only out-of-band override, derived attachment availability/writability, dataset scoping, system schemas, negative-query suppression while membership is open, and production IVM invalidation/recovery. |
