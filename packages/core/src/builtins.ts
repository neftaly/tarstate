import { capabilityRefFor, type CapabilityDeclaration, type CapabilityRegistry } from './registry.js';
import type { CapabilityRef } from './issues.js';

const ref = (suffix: string, hash: string): CapabilityRef => ({ id: 'urn:tarstate:capability:' + suffix, version: '1', contractHash: `sha256:${hash}` });
...
const declaration = (suffix: string, capabilityClass: CapabilityDeclaration['class'], implies: readonly CapabilityRef[] = []): CapabilityDeclaration => ({
