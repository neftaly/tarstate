...
- `createDbStore` for object-backed local state. Commits enforce attached
  object-backed constraints and use delta-backed snapshot materialization
  maintenance, with recompute fallback outside the narrow incremental subset.
...
- `TarstateProvider`, `useTarstateQuery`, `useTarstateQueries`,
  ...
  `useQuery`, `useQueries`, and `useCommit` aliases are also exported.
