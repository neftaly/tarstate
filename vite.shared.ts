...
export const corePublicEntryNames = [
  ...
  'attachment/retained-text-adapter',
  'attachment/text-intent-adapter',
  'capabilities',
  'database',
  'database/adapter',
...
export const automergePublicEntryNames = [
  'repo-lifecycle',
  'system-database',
  'values',
  'view'
...
export const sourceAliasesFor = (repoRoot: string) => [
  ...
  ...automergePublicEntryNames.map((entryName) => sourceAlias(
    repoRoot,
    '@tarstate/automerge/' + entryName,
    'packages/automerge/src/' + entryName + '/index.ts'
  )),
